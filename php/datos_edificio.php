<?php
    ...
    while($row = mysqli_fetch_array($result)) {
        ...
        $query = "SELECT (COUNT(oficina.rut_persona) + (SELECT COUNT(rut_persona) FROM personal_requerido WHERE id_edificio = '$id_edificio')) as personal_edificio FROM oficina JOIN edificio USING(id_edificio) JOIN personal_requerido USING(id_edificio) WHERE id_edificio = '$id_edificio'";
        $personal_edificio = mysqli_query($conexion, $query);

        $query = "SELECT COUNT(permanecer.rut_persona) as personal_requerido_actual FROM permanecer JOIN persona USING(rut_persona) JOIN personal_requerido USING(rut_persona) WHERE personal_requerido.id_edificio = '$id_edificio' AND fecha_salida IS NULL";
